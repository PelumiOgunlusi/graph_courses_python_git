...
```{python}
pa.drop(columns=['personal_id','household_id'], inplace=True)
pa
```
...
```{python}
pa.rename(columns={'oms_recommendation':'who_recommendation'},inplace=True)
pa
```
...
```{python}
pa['status'].replace({'Adulte':'Adult','Enfant':'Child'}, inplace=True)
```
...
```{python}
# to convert seconds per day to minutes per week, we just diivde the second per day columns by 8.571
pa['sedentary_ap_m_wk'] = pa['sedentary_ap_s_day']/8.571
pa['light_ap_m_wk'] = pa['light_ap_s_day']/8.571
pa['mvpa_m_wk'] = pa['mvpa_s_day']/8.571
pa
```

```{python}
pa.drop(columns=['sedentary_ap_s_day','light_ap_s_day','mvpa_s_day'],inplace=True)
pa
```
...
```{python}
cat_phy_act = np.vectorize(cat_phy_act)
pa['total_pa_cat'] = cat_phy_act(pa['total_pa_m_wk'])
pa
```
...
```{python}
px.histogram(pa, x='total_pa_m_wk', title='Distribution of Total activity of participants in minutes per week')
```
...
```{python}
px.box(pa, y='total_pa_m_wk', color='sex', title='Total physical activity distribution determined by gender')
```

Write a short explanation of what you see in the plot.

- The result shows that there the distribution of the total amount of physical activity (light, moderate, and vigorous) carried out in minutes per week across 2 different gender groups.

- We can clearly see a greater number of men are involved in more physical activity compared to women 
...
# 1. Define the assess_risk function

```{python}
def assess_risk2(BMI, TPA):
   if pd.isna(BMI) or pd.isna(TPA):
      return np.nan
   elif BMI >= 30 and TPA < 500:
      return 'High Risk'
   else:
      return 'Not High Risk'
```

# 2. Apply the function to create the risk_category column

```{python}
assess_risk2 = np.vectorize(assess_risk2)
pa['risk_category'] = assess_risk2(pa['bmi'],pa['total_pa_m_wk'])
pa
```

# 3. Percentage stacked bar chart by sex and risk_category

```{python}
fig_percent_stacked = px.histogram(
   pa[pa['risk_category'].notna()],
   x='risk_category',
   color='sex',
   barmode='stack',
   barnorm='percent',
   color_discrete_sequence=['#deb221', '#2f828a'],
   title='Percentage stacked bar chart showing the count of High Risk and Not High Risk individuals, grouped by sex'
)
